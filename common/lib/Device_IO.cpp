...
DEVICE_IO::WriteCacheAndFlush(_In_reads_bytes_(bufferSize) PCHAR pBuffer, _In_ size_t bufferSize)
{
    ...
    else
    {
        ...
        ULONGLONG    cacheWriteEndBlock = cacheWriteBeginBlock + ((m_CacheCurOffset + bufferSize + m_BlockSize) / m_BlockSize);
        size_t       cacheWriteSize = (ULONG)(m_BlockSize  * (cacheWriteEndBlock - cacheWriteBeginBlock));
    ...
    else
    {
        *bytesWritten = 0; // Write() will always pass in a valid pointer for this value
        ...
        { // 3 phase write
            ...
            // Phase 1 - the write begins inside of a block (not at an even block bundary).
            ...
            { // At this point, we have a valid cache, will write as many bytes as possiblle to cache
                size_t writeSize = (bytesRemaining <= GetCacheRemaining()) ? size_t(bytesRemaining) : size_t(GetCacheRemaining());
            ...
            // At this point, there are four possibilities (three resulting from the previous write);
            //  (1) the write begins at a block boundary (the previous operation was skipped),
            //  (2) the write was fully satisfied via the cache, in which case the remaining phases
            //      will not be performed,
            //  (3) the write was partially complete via the cache and there are more bytes to be
            //      written. In this case, the next write will continue at an even block boundary.
            //  (4) one of the functions in phase 1 failed and no bytes were written;
            ...
            // After this point, one (or both) remaining phases will be performed.


            // Phase 2 - if there are more byte to write, the next write will check the [remaining] bytes to
